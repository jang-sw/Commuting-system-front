<template>
    <main>
        <div class="dashboard">
            ...
            <button @click="newUserModalOpen()">신규 등록</button>
            <hr/>
            <table>
                <thead>
                    <tr>
            ...
            <div class="request-container" id="user-modal">
                <form id="userequestForm" class="request-form" @submit="updateUser($event, 0)">
                    <h2>User Update <span style="float: right; color:darkgray; cursor: pointer;" @click="userModalCloase()">X</span></h2>
            ...
            <div class="request-container" id="new-user-modal">
                <form id="userequestForm" class="request-form" @submit="createUser($event, 0)">
                    <h2>Create User <span style="float: right; color:darkgray; cursor: pointer;" @click="newUserModalCloase()">X</span></h2>
                    <div class="input-group">
                        <label for="name">이름</label>
                    ...
                    <div class="input-group">
                        <label for="team">팀</label>
                        <input type="text" id="team" name="team" required v-model="newUser.team">
                    </div>
                    <div class="input-group">
                        <label for="position">직책</label>
                        <input type="text" id="position" name="position" required v-model="newUser.position" >
                    </div>
                    <div class="input-group">
                        <label for="email">이메일</label>
                        <input type="text" id="email" name="email" required v-model="newUser.email">
                    </div>
                    ※초기 비밀번호는 1234
                    <button type="submit" v-bind:disabled="isProcessing">등록</button>
...
<script lang="ts">
    ...
    export default defineComponent({
        ...
        methods: {
            ...
            async createUser(event:Event, retry: number){
                this.isProcessing = true;
                event.preventDefault();
                const token = sessionStorage.getItem('token')
                const userInfo = sessionStorage.getItem('userInfo')
                ...
                return await axios.post(
                    `${this.serverUrl}/adminApi/account/create`,
                    qs.stringify(this.newUser),
                    {
                        headers: { 
                            'content-type': 'application/x-www-form-urlencoded' 
                            , 'accountId' : parsedUserInfo['accountId']
